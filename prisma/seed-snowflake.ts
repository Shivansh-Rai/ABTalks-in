/**
 * Seed 078 LearningProgram "snowflake" from prisma/content/snowflake-cohort JSON.
...
const PROGRAM_SLUG = "snowflake";
const COHORT_SLUG = "snowflake-open";
...
function assertNotProduction(): void {
  ...
  console.log(`[snowflake-seed] targeting host: ${host}`);
}

function loadJsonFile<T>(filename: string): T {
  const full = path.join(CONTENT_DIR, filename);
  if (!fs.existsSync(full)) {
    throw new Error(`[snowflake-seed] ${filename} not found at ${full}`);
  }
  return JSON.parse(fs.readFileSync(full, "utf-8")) as T;
}

function activityIdForDay(dayNumber: number): string {
  return `act_snf_day_${String(dayNumber).padStart(2, "0")}`;
}

function activityIdForVideo(dayNumber: number, order: number): string {
  return `act_snf_vid_${String(dayNumber).padStart(2, "0")}_${String(order).padStart(2, "0")}`;
}

/**
 * snowflake-cohort/videos.json ships real `youtubeId`s from the program doc.
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const category = await prisma.programCategory.upsert({
      ...
      create: {
        slug: PROGRAM_SLUG,
        name: "Snowflake Data & AI Engineering",
        description: "15-Day Snowflake Data & AI Engineering Cohort.",
        colorToken: "snowflake",
        sortOrder: 50,
        isActive: true,
      },
      update: {
    ...
    const program = await prisma.learningProgram.upsert({
      ...
      create: {
        slug: PROGRAM_SLUG,
        title: "15-Day Snowflake Data & AI Engineering Cohort",
    ...
    for (const d of days) {
      ...
      if (!moduleId) {
        throw new Error(
          `[snowflake-seed] day ${d.dayNumber}: module ${d.moduleNumber} not found`,
        );
      }
    ...
    for (const entry of videoDays) {
      ...
      if (!dayMeta) {
        throw new Error(
          `[snowflake-seed] videos: day ${entry.dayNumber} not in days.json`,
        );
      }
      const moduleId = moduleIdByNumber.get(dayMeta.moduleNumber);
      if (!moduleId) {
        throw new Error(
          `[snowflake-seed] videos: module ${dayMeta.moduleNumber} not found`,
        );
      }
    ...
    console.log(
      `[snowflake-seed] upserted program=${PROGRAM_SLUG} modules=${modules.length} days=${days.length} videos=${videoCount} cohort=${COHORT_SLUG}`,
    );
    if (pendingVideoIds > 0) {
      console.warn(
        `[snowflake-seed] ${pendingVideoIds} video(s) have no youtubeId yet — they stay hidden on the day page until curated.`,
      );
    }
  } finally {
