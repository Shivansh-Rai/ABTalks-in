/**
 * Phase 8-D child rehearsal. Production-matching authority flags.
 * ENABLE_DUAL_WRITE=false. Refuses the production host.
 */
...
async function main() {
  ...
  if (host.includes("ep-nameless-term-ams9a5e3")) {
    throw new Error("rehearsal must not target production");
  }
  ...
  const email = `p8d-rehearse-${stamp}@abtalks.dev`;
  const actorEmail = `p8d-rehearse-actor-${stamp}@abtalks.dev`;
  const submissionId = `p8dsub_${stamp}`;
  const report: Record<string, unknown> = { email, stamp };

  const actor = await prisma.user.create({
    data: { email: actorEmail, name: "P8D Actor" },
    select: { id: true },
  });
  const user = await prisma.user.create({
    data: { email, name: "P8D Rehearse" },
    select: { id: true },
  });
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      await applyCandidateIdentityChange(tx, user.id, {
        fullName: "P8D Rehearse Edited",
        githubUsername: "p8d-rehearse",
      });
    }, TX_OPTS);
    ...
    if (cpAfter?.fullName !== "P8D Rehearse Edited") {
    ...
    const idempotencyKey = `p8d-grant:+1:${user.id}:${stamp}`;
      ...
      prisma.$transaction(
        (tx) =>
          applyPointsChange(tx, {
            ...
            idempotencyKey,
            reason: "P8D dual-write-off rehearsal +1",
    ...
    const flagOffKey = `p8d-grant-flagoff:+1:${user.id}:${stamp}`;
    const flagOffGranted = await withLegacyPointsMirrorFlush(() =>
      prisma.$transaction(
        (tx) =>
          applyPointsChange(tx, {
            userId: user.id,
            ...
            sourceType: PointsSourceType.ADMIN_GRANT,
            sourceId: flagOffKey,
            idempotencyKey: flagOffKey,
            reason: "P8D mistaken flag-off still canonical",
            legacyEvent: { type: "COMMUNITY_GRANT" },
          }),
        TX_OPTS,
      ),
    ...
    process.env.ENABLE_NEW_POINTS_WRITES = "true";
    const afterFlagOff = await prisma.pointsAccount.findUnique({
      where: { userId: user.id },
      select: { balance: true },
    });
    const afterFlagOffUser = (
      await prisma.user.findUnique({
        where: { id: user.id },
        select: { synergyPoints: true },
      })
    )?.synergyPoints;
    if (!flagOffGranted.ok || flagOffGranted.appliedAmount !== 1) {
      throw new Error("mistaken ENABLE_NEW_POINTS_WRITES=false must still credit PA");
    }
    if ((afterFlagOff?.balance ?? 0) - (afterPoints.pa?.balance ?? 0) !== 1) {
      throw new Error("flag-off still must write PointsAccount");
    }
    if (afterFlagOffUser !== afterPoints.userSynergy) {
      throw new Error("flag-off restored User.synergyPoints");
    }
    report.mistakenFlagOff = { pointsCanonical: true, legacyFrozen: true };
      ...
      async (tx) =>
        applyProgramMembershipChange(tx, {
          ...
          identity: {
            fullName: "P8D Rehearse",
            githubUsername: "p8d-rehearse",
            githubRepoUrl: "https://github.com/abtalks/p8d-rehearse",
          },
        }),
      TX_OPTS,
    );
