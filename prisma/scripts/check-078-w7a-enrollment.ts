/**
 * W7-A enrollment denorm recon (read-only).
 *
 * Days: Enrollment.daysCompleted vs distinct passed challenge AA days.
 * Streak snapshot: Enrollment vs ProgramEnrollment.track*.
 * Live AA recompute vs stored snapshot is informational (expected ~208).
 * Domain: StudentProfile.domain vs earliest pe_enr_ LearningProgram slug.
 * Mapping: Enrollment ↔ pe_enr_ ProgramEnrollment.
 *
...
async function main() {
  ...
  const unexplainedDays = n([
    { n: day.legacy_higher },
  ]) + n([{ n: day.canonical_higher }]);
  const unexplainedStreak =
    n([{ n: streak.current_mismatch }]) + n([{ n: streak.longest_mismatch }]);
  const unexplainedDomain = n([{ n: dom.value_mismatch }]);
  const mappingErrors =
  ...
  if (isNewEnrollmentStateEnabled() && unexplainedStreak !== 0) {
    process.exitCode = 1;
  }
}
