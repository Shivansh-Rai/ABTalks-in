/**
 * One-shot W1-B production probe: +1 ADMIN_GRANT through applyPointsChange
 * with ENABLE_LEGACY_POINTS_MIRROR=false.
 *
 * Requires PHASE2_ALLOW_PRODUCTION=1, production direct DATABASE_URL,
 * ENABLE_NEW_POINTS_WRITES=true. Does not start W2. Does not drop tables.
 *
 * Target: W1B_GRANT_EMAIL (default arjun@abtalks.dev).
 */
...
const TARGET_EMAIL =
  process.env.W1B_GRANT_EMAIL?.trim() ||
  process.env.W1A_GRANT_EMAIL?.trim() ||
  "arjun@abtalks.dev";
...
async function main() {
  ...
  if (isLegacyPointsMirrorEnabled()) {
    throw new Error("ENABLE_LEGACY_POINTS_MIRROR must be false for this probe");
  }
  ...
  const idempotencyKey = `w1b-prod-grant:+1:${target.id}:${Date.now()}`;
    ...
    prisma.$transaction(
      (tx) =>
        applyPointsChange(tx, {
          ...
          idempotencyKey,
          reason: "W1-B production mirror-off probe (+1)",
  ...
  if (report.deltas.userSynergy !== 0) {
    throw new Error(`User.synergyPoints changed by ${report.deltas.userSynergy}`);
  }
  if (report.deltas.profileSynergy !== 0) {
    throw new Error(
      `StudentProfile.synergyPoints changed by ${report.deltas.profileSynergy}`,
    );
  }
  if (report.deltas.eventCount !== 0) {
    throw new Error(`SynergyEvent changed by ${report.deltas.eventCount}`);
  }
  console.log("W1-B production +1 grant probe passed (legacy frozen).");
}
