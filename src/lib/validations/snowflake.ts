...
export const snowflakeEnrollSchema = z
  .object({
    linkedinUrl: z
      .string()
      .trim()
...
export type SnowflakeEnrollInput = z.infer<typeof snowflakeEnrollSchema>;

export const snowflakeMissionDaySchema = z.object({
  dayNumber: z.number().int().min(1).max(SNOWFLAKE_TOTAL_DAYS),
});

export const snowflakeSubmitMissionSchema = z.object({
  dayNumber: z.number().int().min(1).max(SNOWFLAKE_TOTAL_DAYS),
  payload: z.union([
    z.object({
