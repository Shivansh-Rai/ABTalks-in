...
export const dsArchitectEnrollSchema = z
  .object({
    linkedinUrl: z
      .string()
      .trim()
...
export type DsArchitectEnrollInput = z.infer<typeof dsArchitectEnrollSchema>;

export const dsArchitectMissionDaySchema = z.object({
  dayNumber: z.number().int().min(1).max(DS_ARCHITECT_TOTAL_DAYS),
});

export const dsArchitectSubmitMissionSchema = z.object({
  dayNumber: z.number().int().min(1).max(DS_ARCHITECT_TOTAL_DAYS),
  payload: z.union([
    z.object({
