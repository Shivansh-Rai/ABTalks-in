...
import type { DatabricksAiEnrollInput } from "@/lib/validations/databricks-ai";
import { getProfileSummary, updateCandidateLinks } from "@/repositories/candidate";
import {
  createDatabricksAiEnrollmentRow,
  findDatabricksAiEnrollment,
  getDatabricksAiCohort,
} from "@/repositories/databricks-ai";

export type DatabricksAiEntryState =
  | { screen: "needs_profile" }
  | { screen: "closed" }
  | { screen: "form" }
  | { screen: "enrolled"; enrollmentId: string };

export async function getDatabricksAiEntryState(
  userId: string,
): Promise<DatabricksAiEntryState> {
  const existing = await findDatabricksAiEnrollment(userId);
  if (
    existing &&
...
export async function createDatabricksAiEnrollment(
  userId: string,
  input: DatabricksAiEnrollInput,
  ...
  const enrollmentId = await writeClient().$transaction(async (tx) => {
    const row = await createDatabricksAiEnrollmentRow(tx, {
      userId,
      cohortId: cohort.id,
