...
import {
  DATABRICKS_AI_MAX_RUNS_PER_DAY,
  DATABRICKS_AI_MIN_RUN_INTERVAL_MS,
...
import {
  countDatabricksAiAttemptsForDay,
  getLatestDatabricksAiAttemptAt,
  hasPassedDatabricksAiActivity,
  listDatabricksAiAttemptsForDay,
  listDatabricksAiProgress,
  markDatabricksAiEnrollmentCompleted,
  recordDatabricksAiAttempt,
  recomputeDatabricksAiProgress,
  type DatabricksAiEnrollment,
} from "@/repositories/databricks-ai";
import { getActivityVerificationForDay } from "@/repositories/learning";

export type DatabricksAiMissionState = {
  dayState: DatabricksAiDayState;
  passed: boolean;
...
export type DatabricksAiSubmitOk = {
  passed: boolean;
  verdict: VerdictLine[];
...
export async function getDatabricksAiMissionState(
  enrollment: DatabricksAiEnrollment,
  dayNumber: number,
): Promise<DatabricksAiMissionState | null> {
  ...
  const [progress, attempts] = await Promise.all([
    listDatabricksAiProgress(enrollment.id),
    listDatabricksAiAttemptsForDay(enrollment.id, dayNumber),
  ]);
...
export async function submitDatabricksAiMissionRun(
  enrollment: DatabricksAiEnrollment,
  dayNumber: number,
  payload: unknown,
): Promise<DatabricksAiSubmitOk | { ok: false; message: string }> {
  ...
  const runCount = await countDatabricksAiAttemptsForDay(enrollment.id, dayNumber);
  if (runCount >= DATABRICKS_AI_MAX_RUNS_PER_DAY) {
    return { ok: false, message: "Daily run limit reached for this mission." };
  }
  const latestAt = await getLatestDatabricksAiAttemptAt(enrollment.id, dayNumber);
  if (
    latestAt &&
    Date.now() - latestAt.getTime() < DATABRICKS_AI_MIN_RUN_INTERVAL_MS
  ) {
  ...
  const alreadyPassed = await hasPassedDatabricksAiActivity(
    enrollment.id,
  ...
  await writeClient().$transaction(async (tx) => {
    await recordDatabricksAiAttempt(tx, {
    ...
    await recomputeDatabricksAiProgress(tx, enrollment.id);
    if (dayNumber === DATABRICKS_AI_TOTAL_DAYS && result.passed) {
      await markDatabricksAiEnrollmentCompleted(tx, enrollment.id);
    }
  });

  let unlockedDay: number | undefined;
  if (result.passed && dayNumber < DATABRICKS_AI_TOTAL_DAYS) {
