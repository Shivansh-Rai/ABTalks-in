...
export async function getDatabricksAiDashboard(
  enrollment: DatabricksAiEnrollment,
): Promise<DatabricksAiDashboard> {
  const [catalog, progress, cached, recent] = await Promise.all([
    listCurriculumForProgramSlug(DATABRICKS_AI_PROGRAM_SLUG),
    listDatabricksAiProgress(enrollment.id),
    getDatabricksAiEnrollmentProgress(enrollment.id),
    listRecentDatabricksAiAttempts(enrollment.id, 8),
  ]);
  ...
  const lastDay = progress.find((p) => p.dayNumber === DATABRICKS_AI_TOTAL_DAYS);
  ...
  return {
    ...
    totalDays: DATABRICKS_AI_TOTAL_DAYS,
    missionPoints: cached?.pointsEarned ?? 0,
    maxMissionPoints: DATABRICKS_AI_MAX_MISSION_POINTS,
