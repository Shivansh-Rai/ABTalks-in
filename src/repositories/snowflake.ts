...
const SNF_DAY_ACTIVITY_PREFIX = "act_snf_day_";

export type SnowflakeEnrollment = {
  id: string;
  userId: string;
  cohortId: string;
...
export type SnowflakeAttemptRow = {
  id: string;
  activityId: string;
  dayNumber: number;
...
export async function getSnowflakeCohort(): Promise<{
  id: string;
  status: CohortStatus;
  timezone: string;
} | null> {
  const row = await prisma.cohort.findUnique({
    where: { slug: SNOWFLAKE_COHORT_SLUG },
    select: { id: true, status: true, timezone: true },
  });
  return row;
}

export async function findSnowflakeEnrollment(
  userId: string,
): Promise<SnowflakeEnrollment | null> {
...
export async function createSnowflakeEnrollmentRow(
...
export async function listSnowflakeProgress(
  enrollmentId: string,
...
export async function listSnowflakeAttemptsForDay(
  enrollmentId: string,
  dayNumber: number,
): Promise<SnowflakeAttemptRow[]> {
  const rows = await prisma.activityAttempt.findMany({
    where: {
      enrollmentId,
      activity: {
        dayNumber,
        id: { startsWith: SNF_DAY_ACTIVITY_PREFIX },
      },
    },
...
export async function countSnowflakeAttemptsForDay(
  enrollmentId: string,
  dayNumber: number,
): Promise<number> {
...
export async function listRecentSnowflakeAttempts(
  enrollmentId: string,
  take: number,
): Promise<SnowflakeAttemptRow[]> {
  const rows = await prisma.activityAttempt.findMany({
...
export async function getLatestSnowflakeAttemptAt(
  enrollmentId: string,
  dayNumber: number,
): Promise<Date | null> {
  const row = await prisma.activityAttempt.findFirst({
...
export async function recordSnowflakeAttempt(
  tx: Prisma.TransactionClient,
  input: {
    enrollmentId: string;
    activityId: string;
...
export async function markSnowflakeEnrollmentCompleted(
  tx: Prisma.TransactionClient,
  enrollmentId: string,
): Promise<void> {
...
export async function getSnowflakeEnrollmentProgress(
  enrollmentId: string,
): Promise<{
...
export async function recomputeSnowflakeProgress(
  tx: Prisma.TransactionClient,
  enrollmentId: string,
): Promise<void> {
  ...
  const activities = await tx.activity.findMany({
    where: { id: { startsWith: SNF_DAY_ACTIVITY_PREFIX } },
  ...
  const percentCompleteBp = Math.round(
    (completedActivities / SNOWFLAKE_TOTAL_DAYS) * 10000,
  );
  const now = new Date();
