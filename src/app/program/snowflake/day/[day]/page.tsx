...
import { SnowflakeMissionPanel } from "@/components/snowflake/snowflake-mission-panel";
...
import { getSnowflakeDayShell } from "@/features/snowflake/days";
import { getSnowflakeMissionState } from "@/features/snowflake/missions";
...
export default async function SnowflakeDayPage({ params }: Props) {
  ...
  if (
    ...
    dayNumber < 1 ||
    dayNumber > SNOWFLAKE_TOTAL_DAYS
  ) {
    redirect(SNOWFLAKE_BASE);
  }

  const result = await getSnowflakeDayShell(enrollment, dayNumber);
  ...
  const [missionState, dashboard] = await Promise.all([
    getSnowflakeMissionState(enrollment, dayNumber),
    getSnowflakeDashboard(enrollment),
  ]);
  if (!missionState) redirect(SNOWFLAKE_BASE);
