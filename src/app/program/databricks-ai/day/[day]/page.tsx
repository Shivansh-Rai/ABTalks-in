...
import { DatabricksAiMissionPanel } from "@/components/databricks-ai/databricks-ai-mission-panel";
...
import { getDatabricksAiDayShell } from "@/features/databricks-ai/days";
import { getDatabricksAiMissionState } from "@/features/databricks-ai/missions";
...
export default async function DatabricksAiDayPage({ params }: Props) {
  ...
  if (
    ...
    dayNumber < 1 ||
    dayNumber > DATABRICKS_AI_TOTAL_DAYS
  ) {
    redirect(DATABRICKS_AI_BASE);
  }

  const result = await getDatabricksAiDayShell(enrollment, dayNumber);
  ...
  const [missionState, dashboard] = await Promise.all([
    getDatabricksAiMissionState(enrollment, dayNumber),
    getDatabricksAiDashboard(enrollment),
  ]);
  if (!missionState) redirect(DATABRICKS_AI_BASE);
