...
import { PowerBiMissionPanel } from "@/components/powerbi/powerbi-mission-panel";
...
import { getPowerBiDayShell } from "@/features/powerbi/days";
import { getPowerBiMissionState } from "@/features/powerbi/missions";
...
export default async function PowerBiDayPage({ params }: Props) {
  ...
  if (
    ...
    dayNumber < 1 ||
    dayNumber > POWERBI_TOTAL_DAYS
  ) {
    redirect(POWERBI_BASE);
  }

  const result = await getPowerBiDayShell(enrollment, dayNumber);
  ...
  const [missionState, dashboard] = await Promise.all([
    getPowerBiMissionState(enrollment, dayNumber),
    getPowerBiDashboard(enrollment),
  ]);
  if (!missionState) redirect(POWERBI_BASE);
