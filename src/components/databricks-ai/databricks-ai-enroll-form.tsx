...
import { enrollInDatabricksAiAction } from "@/app/actions/databricks-ai-actions";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import {
  databricksAiEnrollSchema,
  type DatabricksAiEnrollInput,
} from "@/lib/validations/databricks-ai";

type FormInput = z.input<typeof databricksAiEnrollSchema>;
...
export function DatabricksAiEnrollForm() {
  const router = useRouter();
  const [submitting, setSubmitting] = useState(false);
  const [skillInput, setSkillInput] = useState("");

  const form = useForm<FormInput, unknown, DatabricksAiEnrollInput>({
    resolver: zodResolver(databricksAiEnrollSchema),
    defaultValues: {
      skills: [],
      linkedinUrl: "",
      githubUsername: "",
  ...
  async function onSubmit(values: DatabricksAiEnrollInput) {
    setSubmitting(true);
    try {
      const res = await enrollInDatabricksAiAction(values);
      if (!res.ok) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="githubRepoUrl"
            placeholder="https://github.com/you/databricks-ai-cohort"
