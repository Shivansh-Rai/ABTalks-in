...
import type { DatabricksAiMissionState } from "@/features/databricks-ai/missions";
import { DATABRICKS_AI_TOTAL_DAYS } from "@/features/databricks-ai/constants";
import { submitDatabricksAiMissionAction } from "@/app/actions/databricks-ai-actions";
...
type Props = {
  ...
  missionState: DatabricksAiMissionState;
...
export function DatabricksAiMissionPanel({
  dayNumber,
  missionType,
  githubRepoUrl,
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = await submitDatabricksAiMissionAction({
        dayNumber,
        payload,
      });
      if (!result.ok) {
    ...
    return (
      <div id="mission-verify" className={cn(cardClass, "space-y-4")}>
        ...
        {passedBanner?.unlockedDay &&
          passedBanner.unlockedDay <= DATABRICKS_AI_TOTAL_DAYS && (
            <Link
              href={`/program/databricks-ai/day/${passedBanner.unlockedDay}`}
              className={ctaClass}
            >
