...
import { enrollInDsArchitectAction } from "@/app/actions/ds-architect-actions";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import {
  dsArchitectEnrollSchema,
  type DsArchitectEnrollInput,
} from "@/lib/validations/ds-architect";

type FormInput = z.input<typeof dsArchitectEnrollSchema>;
...
export function DsArchitectEnrollForm() {
  const router = useRouter();
  const [submitting, setSubmitting] = useState(false);
  const [skillInput, setSkillInput] = useState("");

  const form = useForm<FormInput, unknown, DsArchitectEnrollInput>({
    resolver: zodResolver(dsArchitectEnrollSchema),
    defaultValues: {
      skills: [],
      linkedinUrl: "",
      githubUsername: "",
  ...
  async function onSubmit(values: DsArchitectEnrollInput) {
    setSubmitting(true);
    try {
      const res = await enrollInDsArchitectAction(values);
      if (!res.ok) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="githubRepoUrl"
            placeholder="https://github.com/you/dsa-cohort"
