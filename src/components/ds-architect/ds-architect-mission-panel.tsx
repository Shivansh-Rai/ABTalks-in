...
import type { DsArchitectMissionState } from "@/features/ds-architect/missions";
import { DS_ARCHITECT_TOTAL_DAYS } from "@/features/ds-architect/constants";
import { submitDsArchitectMissionAction } from "@/app/actions/ds-architect-actions";
...
type Props = {
  ...
  missionState: DsArchitectMissionState;
...
export function DsArchitectMissionPanel({
  dayNumber,
  missionType,
  githubRepoUrl,
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = await submitDsArchitectMissionAction({
        dayNumber,
        payload,
      });
      if (!result.ok) {
    ...
    return (
      <div id="mission-verify" className={cn(cardClass, "space-y-4")}>
        ...
        {passedBanner?.unlockedDay &&
          passedBanner.unlockedDay <= DS_ARCHITECT_TOTAL_DAYS && (
            <Link
              href={`/program/ds-architect/day/${passedBanner.unlockedDay}`}
              className={ctaClass}
            >
