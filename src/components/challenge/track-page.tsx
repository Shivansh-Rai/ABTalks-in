...
export async function TrackPage({ domain, searchParams }: TrackPageProps) {
  ...
  const [allEnrollments, data, isHackathonRegistered] = await Promise.all([
    ...
    getDashboardData(session.user.id, enrollmentForDomain.id),
          ...
          <CardContent className="space-y-4">
            {isChallengeComplete ? (
              ...
              <div className="flex flex-col gap-4 rounded-xl border border-border/60 bg-muted/25 p-6">
                ...
                <Link
                  href={challengeHref(enrollment.id, "/challenge/today")}
                  className={cn(
