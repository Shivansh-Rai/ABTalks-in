...
import type { SnowflakeMissionState } from "@/features/snowflake/missions";
import { SNOWFLAKE_TOTAL_DAYS } from "@/features/snowflake/constants";
import { submitSnowflakeMissionAction } from "@/app/actions/snowflake-actions";
...
type Props = {
  ...
  missionState: SnowflakeMissionState;
...
export function SnowflakeMissionPanel({
  dayNumber,
  missionType,
  githubRepoUrl,
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = await submitSnowflakeMissionAction({
        dayNumber,
        payload,
      });
      if (!result.ok) {
    ...
    return (
      <div id="mission-verify" className={cn(cardClass, "space-y-4")}>
        ...
        {passedBanner?.unlockedDay &&
          passedBanner.unlockedDay <= SNOWFLAKE_TOTAL_DAYS && (
            <Link
              href={`/program/snowflake/day/${passedBanner.unlockedDay}`}
              className={ctaClass}
            >
