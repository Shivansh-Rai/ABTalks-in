...
import {
  ...
  type SnowflakeEnrollInput,
} from "@/lib/validations/snowflake";

type FormInput = z.input<typeof snowflakeEnrollSchema>;
...
export function SnowflakeEnrollForm() {
  ...
  const form = useForm<FormInput, unknown, SnowflakeEnrollInput>({
    resolver: zodResolver(snowflakeEnrollSchema),
    defaultValues: {
      skills: [],
      linkedinUrl: "",
      githubUsername: "",
  ...
  async function onSubmit(values: SnowflakeEnrollInput) {
    setSubmitting(true);
    try {
      const res = await enrollInSnowflakeAction(values);
      if (!res.ok) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="githubRepoUrl"
            placeholder="https://github.com/you/snowflake-cohort"
