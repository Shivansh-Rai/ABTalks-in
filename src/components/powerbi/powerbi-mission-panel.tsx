...
import type { PowerBiMissionState } from "@/features/powerbi/missions";
import { POWERBI_TOTAL_DAYS } from "@/features/powerbi/constants";
import { submitPowerBiMissionAction } from "@/app/actions/powerbi-actions";
...
type Props = {
  ...
  missionState: PowerBiMissionState;
...
export function PowerBiMissionPanel({
  dayNumber,
  missionType,
  githubRepoUrl,
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = await submitPowerBiMissionAction({
        dayNumber,
        payload,
      });
      if (!result.ok) {
      ...
      <div id="mission-verify" className={cn(cardClass, "space-y-4")}>
        ...
        {passedBanner?.unlockedDay &&
          passedBanner.unlockedDay <= POWERBI_TOTAL_DAYS && (
            <Link
              href={`/program/powerbi/day/${passedBanner.unlockedDay}`}
              className={ctaClass}
            >
