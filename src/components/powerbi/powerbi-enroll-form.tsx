...
import { enrollInPowerBiAction } from "@/app/actions/powerbi-actions";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import {
  powerBiEnrollSchema,
  type PowerBiEnrollInput,
} from "@/lib/validations/powerbi";

type FormInput = z.input<typeof powerBiEnrollSchema>;
...
export function PowerBiEnrollForm() {
  const router = useRouter();
  const [submitting, setSubmitting] = useState(false);
  const [skillInput, setSkillInput] = useState("");

  const form = useForm<FormInput, unknown, PowerBiEnrollInput>({
    resolver: zodResolver(powerBiEnrollSchema),
    defaultValues: {
      skills: [],
      linkedinUrl: "",
      githubUsername: "",
  ...
  async function onSubmit(values: PowerBiEnrollInput) {
    setSubmitting(true);
    try {
      const res = await enrollInPowerBiAction(values);
      if (!res.ok) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="githubRepoUrl"
            placeholder="https://github.com/you/pbi-cohort"
