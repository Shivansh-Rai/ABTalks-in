...
> 7. **Added beyond the plan — OAuth signup consent.** The plan assumed the seven signup forms covered every entry point. They do not: Google OAuth creates the `User` + `Account` rows before any form is reached, so a visitor could sign in, abandon registration, and leave us holding their email, name and photo with no consent record — the exact thing DPDP requires consent *before*. Fixed by a Terms/Privacy/18+ notice on `/login` (a notice, not a checkbox: returning users sign in repeatedly and have already accepted) plus `events.createUser` in `src/auth.ts`, which fires once on first account creation and writes consent under a new `oauth_signup` source. Wrapped in try/catch so a missing `LegalConsent` table can never break sign-in. `auth.config.ts` untouched — it must stay edge-safe.
> 8. **Merged `fix/build-safety-and-dashboard` into this branch** so the working tree carries the double-progress-bar fix, the Auth.js Edge secret, and the build/migration decoupling. Without it, checking out this branch reintroduced the two-progress-bar bug on the dashboard.
>
